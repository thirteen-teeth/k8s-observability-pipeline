...
```
kubectl -n kafka exec -it teeth-queue-kafka-0 -c kafka -- bin/kafka-topics.sh --bootstrap-server teeth-queue-kafka-brokers.kafka.svc.cluster.local:9092 --list
kubectl -n kafka exec -it teeth-queue-kafka-0 -- bin/kafka-console-consumer.sh --bootstrap-server teeth-queue-kafka-brokers.kafka.svc.cluster.local:9092 --topic otlp_logs
```